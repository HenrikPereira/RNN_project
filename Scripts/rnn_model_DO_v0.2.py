...
# %%
# From https://github.com/Tony607/Keras_TimeseriesGenerator/blob/master/TimeseriesGenerator.ipynb
...
def make_rnn(
    ...
    rnn0 = SimpleRNN(train_ts.shape[-1], activation='relu', return_sequences=True)(seq_input)
...
#%%
model_rnn.predict_generator(val_gen, steps=100).shape
...
#
   # return autoencoder_, encoder_, model_full_
    return ae0, decode, encode,seq_input#, concat0,rnn0, out
...
ae0, decode, encode,seq_input= make_ae_rnn(
    product_shape=train_prod_feat.shape
)
#%%Reshape do seq_input para 2d e conversão para numpy array
seq_input_v2 = tf.reshape(seq_input,(length, train_ts.shape[-1]))

#w = tf.placeholder(tf.float32, shape=[None,12,751], name="teste_3")

#x = seq_input_v2.eval(session=K.get_session(),feed_dict={seq_input_v2:np.zeros(12,751)})
